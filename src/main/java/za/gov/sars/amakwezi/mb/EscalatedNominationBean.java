...
@ManagedBean
@ViewScoped
public class EscalatedNominationBean extends BaseBean<Nomination> {
    ...
    private List<User> rerouteManagers = new ArrayList<>();
    ...
    public void init() {
        ...
        setPanelTitleName("Escalated Nominations");
        Pageable pageable = PageRequest.of(0, 15, Sort.by("referenceId").ascending());

        slices = nominationService.findEscalatedNominations(pageable);
    ...
    public void reRouteNomination(Nomination nomination) {
        ...
        if (nomination.getCostCentreManagerStatus() == CostCentreManagerStatus.APPROVED) {
            if (nomination.getCostCentreManagerStatus() == CostCentreManagerStatus.APPROVED && nomination.getFinanceManagerStatus() != FinanceManagerStatus.APPROVED) {
                managers.addAll(userService.findByUserRole(userRoleService.findByDescription("Finance Manager")));
                //TO DO... merge the two calls to DB into one call
                managers.addAll(userService.findByUserRole(userRoleService.findByDescription("Line and Finance Manager")));
            }
        } else {
            managers.addAll(userService.findByUserRole(userRoleService.findByDescription("Cost Centre Manager")));
            //TO DO... merge the two calls to DB into one call
            managers.addAll(userService.findByUserRole(userRoleService.findByDescription("Line and Cost Centre Manager")));
        }
...
//send reroute email to cost center
  ...
  //      String destinationAddress = lDAPService.getUserEmailAddress("s2026064");
        ...
        builder.append("The below nomination for On-the-Spot Rewards and Recognition was rerouted to alternative approver, please see the details below.");
        builder.append("<br /><br />");
        builder.append("Nomination information");
        builder.append("<br />");
        builder.append("<table border=1>");
    ...
    //send Notitification to CostCentre manager after from finanace manager
